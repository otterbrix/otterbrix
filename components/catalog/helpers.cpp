...
namespace components::catalog {
    ...
    std::vector<oid_t> parse_oid_csv(const std::string& s, bool& ok) {
        ...
        std::size_t i = 0;
        // Not `while (i < s.size())`: that stops as soon as the last comma is the final
        // character, so the empty token behind it -- the only trace of a list TRUNCATED at a
        // separator -- is never looked at. "7,11,13" cut down to "7,11," would read back as a
        // clean two-element list that agrees with every length guard downstream. Every token,
        // including the one after the final separator, is visited.
        for (;;) {
            ...
            if (tok.empty()) {
                ...
                // Read STRAIGHT INTO oid_t: from_chars answers result_out_of_range for a value
                // the target type can't hold, which is the point -- reading into a wider
                // integer and casting down would turn 2^32 + N into N, silently binding the
                // key to whichever column oid N happens to be, with no length guard able to
                // see the swap.
                oid_t v{};
