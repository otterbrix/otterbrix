...
namespace {
    // Indexes are oid-identified; there is no file to name -- the facade opens nothing.
    ...
    // its disk agent owns, so there is no file and no handle to build here.
    //
    // There is no second arm to this factory any more. It used to build an in-memory
    // hash_single_field_index_t beside the disk facade and run every contract below over
    // BOTH, because a table without a disk catalog got the in-memory one. That class is
    // gone with C6a: an index keeps its rows on disk, full stop, and manager_index_t
    // refuses CREATE INDEX outright when there is no catalog path to keep them under.
    ...
    // The rows this index answers with for `= key`, asked through the door it actually has.
    //
    // The disk facade keeps only the PENDING half and its local doors are terminal, because
    // a door that answered from that half alone would return a SUBSET of the rows the key
    // carries and no caller could tell (rule 6). Its production door is
    // merge_uncommitted_rows, which manager_index_t calls with the committed half the disk
    // agent returned; these cases have no agent, so the half handed in is empty.
    ...
    // not yet mirrored to disk".
    constexpr uint64_t committed_bucket = 0;

    void insert_row(index_t& index, const components::types::logical_value_t& value, int64_t row_index) {
    ...
    // Every entry the facade is holding for one txn bucket, in no particular order.
    std::vector<int64_t> pending_insert_rows(const index_t& index, uint64_t txn_id) {
...
TEST_CASE("disk_single_field_index:base") {
    ...
    for (const auto& [value, row_idx] : data) {
        ...
        insert_row(*index, val, row_idx);
    }

    components::types::logical_value_t value(&resource, static_cast<int64_t>(10));
    auto rows = search_rows(*index, components::expressions::compare_type::eq, value, 0);
    REQUIRE(rows.size() == 1);
    REQUIRE(rows.front() == 2);

    components::types::logical_value_t missing(&resource, static_cast<int64_t>(11));
    REQUIRE(search_rows(*index, components::expressions::compare_type::eq, missing, 0).empty());

    for (const auto& [data_value, row_idx] : data) {
        components::types::logical_value_t val(&resource, data_value);
        insert_row(*index, val, row_idx + 100);
    }
    rows = search_rows(*index, components::expressions::compare_type::eq, value, 0);
    REQUIRE(rows.size() == 2);
...
TEST_CASE("disk_single_field_index:engine") {
    ...
    const uint32_t id = make_index<disk_hash_single_field_index_t>(index_engine, 201u, {key(&resource, "count")});

    auto* idx = search_index(index_engine, id);
    REQUIRE(idx != nullptr);

    insert_row(*idx, components::types::logical_value_t(&resource, 0), int64_t(0));
    for (int i = 10; i >= 1; --i) {
        insert_row(*idx, components::types::logical_value_t(&resource, i), int64_t(11 - i));
    }

    // EVERY write landed, not just the one probed below. An index that dropped a row on
    // the floor would still answer the single probe correctly, so the count is what makes
    // the case fail on a lossy write door. The in-memory index this replaced was counted
    // by walking cbegin()/cend(); a disk facade holds no such sequence, and the bucket it
    // does hold is the same eleven entries.
    REQUIRE(pending_insert_rows(*idx, committed_bucket).size() == 11);

    components::types::logical_value_t value(&resource, 5);
    auto rows = search_rows(*idx, components::expressions::compare_type::eq, value, 0);
    REQUIRE(rows.size() == 1);
...
// the whole answer (rule 6) -- so all of them are terminal now. The testable contract is
// therefore the capability: reads_through_disk_agent() is what manager_index_t consults
// before it decides to send the read to the agent instead of calling find(), and it is
// what keeps callers off those doors.
//
// NOTE ON THE MISSING COUNTER-EXAMPLE: this case used to hold an in-memory index beside
// the facade and require FALSE of it, so the predicate was shown to distinguish
// implementations rather than answer true for everything. After C6a there is no
// implementation left that reads anywhere but through an agent, so no such counter-example
// can be written. The predicate is true of every surviving index by construction.
TEST_CASE("disk_single_field_index:read_only_facade_direct_ops_do_not_materialize_committed_state") {
    ...
    auto own_before_commit = search_rows(*index, components::expressions::compare_type::eq, value, txn_insert);
    ...
    auto other_after_commit = search_rows(*index, components::expressions::compare_type::eq, value, txn_other);
...
TEST_CASE("disk_single_field_index:pending_insert_delete_and_txn_state") {
    ...
    auto visible_own_txn = search_rows(*index, components::expressions::compare_type::eq, key, txn_insert);
    ...
    auto hidden_for_delete_txn = search_rows(*index, components::expressions::compare_type::eq, key, txn_delete);
...
TEST_CASE("disk_single_field_index:find_reads_disk_and_normalizes_integer_keys") {
    ...
    auto eq_rows = search_rows(*index, components::expressions::compare_type::eq, key_smallint, 0);
    ...
    REQUIRE(search_rows(*index, components::expressions::compare_type::eq, other, 0).empty());
}
...
TEST_CASE("disk_single_field_index:lower_upper_bound_not_supported") {
    ...
    auto on_disk = make_hash_index(&resource);
    ...
    // distinguishes implementations rather than answering false for everything. It used to
    // be the in-memory ordered index; the disk-backed ordered facade is the one that
    // answers a range on the real path now, and it is the one the predicate has to
    // separate from this file's subject.
    disk_ordered_single_field_index_t ordered(&resource,
                                              test_index_oid,
                                              keys_base_storage_t{key(&resource, "count")});
