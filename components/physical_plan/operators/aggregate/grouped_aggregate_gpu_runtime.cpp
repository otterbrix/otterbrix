...
namespace components::operators::aggregate::gpu {
    ...
    void opencl_runtime_t::init() {
        ...
        // OTTERBRIX_GROUP_AGG_GPU_ANY_DEVICE=1 lets non-GPU OpenCL runtimes (e.g. POCL)
        // satisfy the device search. Used to exercise the OpenCL path under ASAN,
        // where the NVIDIA driver is incompatible with ASAN's shadow memory.
        const auto device_type_filter =
            read_bool_env("OTTERBRIX_GROUP_AGG_GPU_ANY_DEVICE") ? cl_device_type_all : cl_device_type_gpu;

        cl_platform_id selected_platform = nullptr;
        for (auto platform : platforms) {
            cl_uint device_count = 0;
            const auto status = api_.clGetDeviceIDs(platform, device_type_filter, 0, nullptr, &device_count);
            if ((status == cl_success || status == cl_device_not_found) && device_count > 0) {
                std::vector<cl_device_id> devices(device_count);
                if (api_.clGetDeviceIDs(platform, device_type_filter, device_count, devices.data(), nullptr) ==
                    cl_success) {
    ...
    bool enqueue_read_buffer_async(opencl_runtime_t& runtime,
        ...
        if (runtime.api().clEnqueueReadBuffer(runtime.queue(),
                                              ...
                                              nullptr,
                                              nullptr) != cl_success) {
            runtime.set_last_error(fmt::format("failed to enqueue read of OpenCL buffer '{}'", label));
            return false;
        }
        return true;
    }

    bool flush_queue(opencl_runtime_t& runtime) {
        if (runtime.api().clFinish(runtime.queue()) != cl_success) {
            runtime.set_last_error("failed to synchronize OpenCL command queue");
            return false;
        }
        return true;
    }
    ...
    bool enqueue_1d_kernel(opencl_runtime_t& runtime, cl_kernel kernel, uint64_t row_count) {
        if (row_count == 0) {
            return true;
        }

        const size_t local_size = default_local_size;
        const size_t threads_needed = std::min(static_cast<size_t>(row_count), max_gpu_threads);
        const size_t global_size = ((threads_needed + local_size - 1) / local_size) * local_size;
