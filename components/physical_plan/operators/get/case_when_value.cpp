...
namespace components::operators::get {
    ...
    operator_get_ptr
    case_when_value_t::create(const std::pmr::vector<expressions::param_storage>& params,
        ...
        while (i + 1 < params.size()) {
            ...
            auto cond_key = cond->primary_key();
            auto cond_cmp = cond->type();
            auto cond_val = (cond_cmp == expressions::compare_type::is_null ||
                             cond_cmp == expressions::compare_type::is_not_null)
                ? types::logical_value_t(std::pmr::get_default_resource(),
                                                                 types::complex_logical_type{types::logical_type::NA})
                : storage_params->parameters.at(cond->value());
            ...
            if (std::holds_alternative<expressions::key_t>(params[i])) {
                ...
                result_constants.push_back(storage_params->parameters.at(
                    std::get<core::parameter_id_t>(params[i])));
            }
        ...
        return operator_get_ptr(new case_when_value_t(
            std::move(result_keys), std::move(result_constants),
            std::move(clauses), e_type, e_index));
    }
    ...
    case_when_value_t::lookup_column(const expressions::key_t& key,
        ...
        for (size_t i = 0; i < key.storage().size(); i++) {
            ...
            if (it == local_values + size) {
                auto* resource = row.empty() ? std::pmr::get_default_resource()
                                             : row.get_allocator().resource();
    ...
    types::logical_value_t
    case_when_value_t::get_result(when_clause::result_kind kind, size_t index,
                                  const std::pmr::vector<types::logical_value_t>& row) const {
