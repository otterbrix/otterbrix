...
namespace components::operators {
            ...
            for (auto op : kOps) {
                ...
                return {new predicates::simple_predicate(r,
                    [col_name, const_val, col_is_rhs, op_str](
                        ...
                        if (!vec) return true;
                        if (!vec->validity().row_is_valid(idx)) return false;
                        ...
                        if (op_str == ">")  return cmp == Cmp::more;
                        if (op_str == "<")  return cmp == Cmp::less;
                        if (op_str == ">=") return cmp == Cmp::more || cmp == Cmp::equals;
                        if (op_str == "<=") return cmp == Cmp::less || cmp == Cmp::equals;
                        if (op_str == "=")  return cmp == Cmp::equals;
                        if (op_str == "<>") return cmp != Cmp::equals;
                        return true;
                    })};
            }
    ...
    operator_check_constraint_t::operator_check_constraint_t(
        ...
        for (auto& [name, expr_str] : check_exprs) {
            check_predicates_.emplace_back(std::move(name),
                                            build_check_predicate(resource, expr_str));
        }
    }
