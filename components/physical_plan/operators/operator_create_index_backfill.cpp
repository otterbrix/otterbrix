...
namespace components::operators {
    ...
    void operator_create_index_backfill_t::on_execute_impl(pipeline::context_t* /*ctx*/) {
        async_wait();
    }

    actor_zeta::unique_future<void>
    operator_create_index_backfill_t::await_async_and_resume(pipeline::context_t* ctx) {
        ...
        if (id_index == components::index::INDEX_ID_UNDEFINED) {
            set_error("index already exists");
            co_return;
        }
            ...
            if (total_rows > 0) {
                ...
                if (scan_data) {
                    ...
                    auto [_ir, irf] = actor_zeta::send(ctx->index_address,
                                                        ...
                                                        services::index::execution_context_t{ctx->session, ctx->txn, collection_},
                                                        std::move(scan_data),
                                                        uint64_t{0},
                                                        count);
        ...
        if (ctx->disk_address != actor_zeta::address_t::empty_address() &&
            ...
            auto [_d, df] = actor_zeta::send(ctx->disk_address,
                                              ...
                                              exec_ctx, pg_idx_coll,
                                              std::int64_t{0}, index_oid_);
            co_await std::move(df);
            if (ctx->txn.transaction_id != 0) ctx->pg_catalog_delete_tables.insert(pg_idx_coll);

            auto valid_row = components::catalog::build_pg_index_row(
                resource(), index_oid_, table_oid_, indkey_, /*indisvalid=*/true);
            auto [_w, wf] = actor_zeta::send(ctx->disk_address,
                                              &services::disk::manager_disk_t::append_pg_catalog_row,
                                              exec_ctx, pg_idx_coll, std::move(valid_row));
