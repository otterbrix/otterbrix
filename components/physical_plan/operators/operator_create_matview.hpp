...
namespace components::operators {
    ...
    // Composite physical operator for CREATE MATERIALIZED VIEW ... WITH NO DATA (relkind='m').
    //
    ...
    //
    // It does NOT populate the matview, and it does not carry the body plan either — a parameter it cannot
    // drive would read like a feature. Driving a scan from inside this operator's own await hits an
    // actor_zeta nested-await failure, and the alternative — lowering CREATE to sequence_t(create, insert) —
    // needs an insert whose column bindings are stamped by validate/enrich, both of which run BEFORE the
    // planner mints the matview's oid and against a relation that does not exist yet. So the form that needs
    // population (the implicit WITH DATA) is refused in the transformer
    // (components/sql/transformer/impl/transform_matview.cpp).
    //
    // Pipeline-canonical: dispatched as a single logical_plan node (create_matview_t) → planner stamps
    // catalog_writes + mv_oid → physical_plan_generator builds this operator. No re-parsing in the
    // dispatcher, no follow-up plan dispatches.
