    ...
    namespace {
        ...
        try_extract_inner_equi_keys(const expressions::expression_ptr& expression,
            ...
            if (lhs_left.has_value() && rhs_right.has_value()) {
                return equi_join_key_info_t{*lhs_left, *rhs_right, expressions::key_t(resource, rhs_key.as_string())};
            }
            if (rhs_left.has_value() && lhs_right.has_value()) {
                return equi_join_key_info_t{*rhs_left, *lhs_right, expressions::key_t(resource, lhs_key.as_string())};
            }
            return std::nullopt;
        }
    } // namespace
