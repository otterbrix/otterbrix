...
namespace components::operators {
    ...
    actor_zeta::unique_future<void> operator_resolve_constraint_t::await_async_and_resume(pipeline::context_t* ctx) {
        ...
        if (!pending_unique_attoids.empty()) {
            ...
            std::pmr::vector<types::logical_value_t> attr_vals(resource_);
            attr_vals.emplace_back(types::logical_value_t(resource_, table_oid));
            auto [_u, fut_attr_u] =
                actor_zeta::send(ctx->disk_address,
                                 &services::disk::manager_disk_t::read_chunks_by_key,
                                 exec_ctx,
                                 kPgAttribute,
                                 std::move(attr_keys),
                                 components::operators::make_key_chunk(resource_, std::move(attr_vals)));
