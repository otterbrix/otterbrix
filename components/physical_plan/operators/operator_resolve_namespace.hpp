...
namespace components::operators {
    ...
    public:
        ...
        // all-sink chain; is_streaming_pipeline admits it and drives this operator's
        // await_async_and_resume via the bottom-up needs_async_finalize pass (the
        // single-resolve case is a sourceless sink-root, a multi-resolve case a
        // sourceless all-sink chain). The async scan emits a single-row chunk into
        // output_ and stamps the resolved oid onto target_node_; push()/finalize()
        // inherit the no-op defaults (no consumer reads the chunk as pipeline data —
        // the metadata handoff is the node stamp, read later via plan_resolve_index).
