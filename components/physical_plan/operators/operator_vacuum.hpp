...
namespace components::operators {
    ...
    //
    // Steps: 1) manager_disk_t::vacuum_all (cleanup_versions per storage).
    // 2) manager_index_t::cleanup_all_versions. 3) pg_computed_column GC + physical column
    // compaction for relkind='g'.
    //
    // No index rebuild: compact() is what moves row ids, and its only call site (checkpoint_inner)
    // is a route VACUUM doesn't take.
    //
