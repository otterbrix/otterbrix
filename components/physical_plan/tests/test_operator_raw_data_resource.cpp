...
//
// The multi-chunk constructor takes a std::pmr::vector of chunks, and that vector names
// its arena in its allocator whether or not it holds a single chunk. The empty-vector
// branch used to reach for std::pmr::get_default_resource() instead (rule 14 forbids it
// in production code): the operator's output_ — and every chunk a drain of this source
// later builds — then lived on the global default arena nobody owns or traces, while the
// caller's arena showed nothing. The vector's own allocator is the answer that is right
// in both branches; the non-empty control below pins that the fix did not move the
// populated case off the chunks' arena.
