    ...
    components::operators::operator_ptr create_plan(const context_storage_t& context,
        ...
        switch (node->type()) {
            case node_type::aggregate_t:
                // Aggregate-pushdown (SEAM B) is lowered INSIDE create_plan_aggregate: when the
                // group child is stamped pushdown() and the shape is POD-representable it returns a
                // full_scan CARRYING the reduce spec (the owning agent reduces its slice); otherwise
                // it builds the normal coordinator aggregate chain. No separate routing here.
