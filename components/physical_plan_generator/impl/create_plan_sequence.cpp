...
namespace services::planner::impl {
    ...
    create_plan_sequence(const context_storage_t& context,
        ...
        std::vector<components::operators::operator_ptr> ops;
        ops.reserve(node->children().size());
        for (const auto& child : node->children()) {
            ops.push_back(create_plan(context, function_registry, child, {}, params));
        }
        return boost::intrusive_ptr(
            new components::operators::operator_sequence_t(
                context.resource,
                context.log.clone(),
                std::move(ops)));
    }
