...
def load_consumer_cases(samples_dir: Path) -> tuple[dict[str, list[dict]], set[str], list[str]]:
    compatible_dir = samples_dir / "datafusion_consumer" / "compatible"
    unsupported_dir = samples_dir / "datafusion_consumer" / "unsupported"
    ...
    unsupported = []
    for case in read_json_samples(unsupported_dir):
        name = case.get("name")
        if not name:
            raise AssertionError(f"{case.get('_sample_file')}: unsupported sample needs 'name'")
        unsupported.append(name)

    return expected_results, order_sensitive, unsupported
...
def require_consumer_rejection(name: str, export_dir: Path) -> dict:
    ctx = make_datafusion_context()
    try:
        consume_binary(ctx, export_dir / f"{name}.bin")
    except Exception as exc:  # noqa: BLE001 - consumer-specific rejection details vary by version.
        print(f"PASS DataFusion rejected unsupported {name}.bin: {exc}")
        return {"name": name, "status": "passed", "reason": str(exc)}
    raise AssertionError(f"{name}.bin: DataFusion unexpectedly consumed an unsupported OtterBrix plan")
...
def main() -> int:
    ...
    expected_results, order_sensitive, unsupported_exports = load_consumer_cases(args.samples_dir)
    ...
    unsupported_results = []
    for name in unsupported_exports:
        unsupported_results.append(require_consumer_rejection(name, export_dir))
    ...
    write_report(
        ...
        build_report(
            ...
            comparisons,
            {
                ...
                "unsupported_rejections": unsupported_results,
            },
        ),
    )
