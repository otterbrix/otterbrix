...
#define nodeTag(nodeptr) ((reinterpret_cast<const Node*>(nodeptr))->type)
...
#else
#define castNode(_type_, nodeptr) (reinterpret_cast<_type_*>(nodeptr)) // usused
...
* nodes/equalfuncs.c
*/
extern bool equal(const void* a, const void* b); // mdxn: used only once - gram.y 16824
...
#define IS_OUTER_JOIN(jointype)                                                                                        \
    (((1 << (jointype)) &                                                                                              \
      ((1 << JOIN_LEFT) | (1 << JOIN_FULL) | (1 << JOIN_RIGHT) | (1 << JOIN_ANTI) | (1 << JOIN_LASJ_NOTIN))) != 0)
