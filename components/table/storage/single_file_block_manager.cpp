...
namespace components::table::storage {
    ...
    uint64_t single_file_block_manager_t::free_block_id() {
        ...
        // Draws from reusable_ only, never pending_free_; a live id found here is disk corruption, not an assert.
    ...
    // Latches rather than rolling back: propagation alone leaves degraded() false, letting compact retry forever
    // (measured +18 blocks/round on a 12k-row table).
    ...
    void single_file_block_manager_t::mark_as_free(uint64_t block_id) {
        // Not an assert(): ids are disk-fed. max_block_ is the issuance mark (can trail the file's real extent), not
        // the domain limit.
    ...
    // Called only once the header is proven durable; chain pointers and data blocks are adopted together so a
    // crash can't recover a mismatched pair.
    ...
    // free = {root N's blocks, its metadata chain, its free-list chain} - {root N+1's blocks} - {registry-live ids}.
    // Freed ids land in pending_free_, not reusable_: the file holds TWO copies until the new root is durable.
    ...
    // A failed header write leaves unregistered blocks stuck in issued_since_root_ forever (measured ~655 KB per
    // round, 7.8 MB table); releasable ids move from issued_since_root_ to reusable_, not pending_free_.
    ...
    // A torn 4 KiB write can leave a CRC-valid new-generation header on disk while write() reports failure.
    core::result_wrapper_t<bool>
    single_file_block_manager_t::reconcile_failed_header_write(uint64_t next_iteration, bool write_ok, bool sync_ok) {
        ...
        if (active.iteration == iteration_) {
            // Deliberately NOT latched, or a transient ENOSPC becomes permanent degradation.
            roll_back_uncommitted_round();
            return core::error_t(core::error_code_t::io_error,
        ...
        // Must NOT roll back: an unconfirmed, landed-looking slot may still be the recovered root.
        durable_root_indeterminate_ = true;
        return latch_durability_error(core::error_t(
            core::error_code_t::io_error,
    ...
    core::result_wrapper_t<meta_block_pointer_t> single_file_block_manager_t::serialize_free_list() {
        ...
        // Allocated before the snapshot (of the very pool free_block_id draws from), to avoid reallocating mid-write.
        const uint64_t sub_block_size = meta_mgr.sub_block_size();
        const uint64_t payload_bytes = (candidate_count + 1) * sizeof(uint64_t);
