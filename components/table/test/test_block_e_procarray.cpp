...
TEST_CASE("Block E committed_version_operator respects in_flight_snapshot",
    ...
    bool still_contains_t2 = std::find(data_again.in_flight_snapshot.begin(),
                                        data_again.in_flight_snapshot.end(),
                                        commit_id_t2) != data_again.in_flight_snapshot.end();
    ...
    bool t3_contains_t2 = std::find(data3.in_flight_snapshot.begin(),
                                     data3.in_flight_snapshot.end(),
                                     commit_id_t2) != data3.in_flight_snapshot.end();
...
TEST_CASE("Block E transaction_t::data() snapshot caching",
          "[block_e][procarray]") {
    using namespace components::table;
    using namespace components::session;
...
TEST_CASE("Block E cross-agent atomic visibility (Version B*)",
    ...
    auto visible = [](const transaction_manager_t::snapshot_t& snap,
        ...
        if (id > snap.snapshot_horizon) return false;        // post-snapshot
        if (std::binary_search(snap.in_flight_snapshot.begin(),
                                snap.in_flight_snapshot.end(),
                                id)) return false;            // in-flight
        return true;
    };
    ...
    bool s1_has_t1 = std::find(snap_S1.in_flight_snapshot.begin(),
                                ...
                                commit_id_t1) != snap_S1.in_flight_snapshot.end();
    bool s1_has_t2 = std::find(snap_S1.in_flight_snapshot.begin(),
                                snap_S1.in_flight_snapshot.end(),
                                commit_id_t2) != snap_S1.in_flight_snapshot.end();
    REQUIRE(s1_has_t1);
    REQUIRE(s1_has_t2);
