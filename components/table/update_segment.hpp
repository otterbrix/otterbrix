...
namespace components::table {
    ...
    void update_segment_t::merge_update_loop_internal(update_info_t& base_info,
        ...
        // `counter`, not `count`: `count` is how many tuples THIS update carries and is the
        // drain loop's own bound — advancing it inside the loop would keep it from
        // terminating and walk `aidx` off the end of `indexing`/`ids`. `counter` is the
        // running position in the merged output, the value the merge loop advances. The
        // third argument of pick_new/pick_old is unused by both lambdas; it is passed for
        // symmetry with the merge loop.
        for (; aidx < count; aidx++) {
            auto a_index = indexing.get_index(aidx);
