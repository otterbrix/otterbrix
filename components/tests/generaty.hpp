...
// result boundary, and it belongs to whoever holds the result. The trap a
// hand-rolled lookup falls into: bounding the loop by cur.column_count() (the
// cursor's columns_) while reading the name out of cur.chunks().front().data[i]
// (the chunk) — two descriptors of one result, read as if they were the same
// one. They agree for a cursor built from a chunk, and disagree the moment they
// do not (a cursor built from a schema alone holds an empty chunk, so every
// read walks off the end of an empty vector).
//
