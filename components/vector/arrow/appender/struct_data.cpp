...
namespace components::vector::arrow::appender {
    ...
    core::error_t arrow_struct_data_t::initialize(arrow_append_data_t& result,
    ...
    core::error_t arrow_struct_data_t::append(arrow_append_data_t& append_data,
                                              vector_t& input,
                                              uint64_t from,
                                              uint64_t to,
                                              uint64_t input_size) {
        ...
        for (uint64_t child_idx = 0; child_idx < children.size(); child_idx++) {
            ...
            if (auto error = child_data.append_vector(child_data, *child, from, to, size); error.contains_error()) {
                return error;
            }
        }
    ...
    core::error_t arrow_struct_data_t::finalize(arrow_append_data_t& append_data,
                                                const types::complex_logical_type& type,
                                                ArrowArray* result) {
        result->n_buffers = 1;
        ...
        for (uint64_t i = 0; i < child_types.size(); i++) {
            ...
            auto child_array = arrow_appender_t::finalize_child(child_type, std::move(append_data.child_data[i]));
            if (child_array.has_error()) {
                return child_array.error();
            }
            append_data.child_arrays[i] = *child_array.value();
        }
        return core::error_t::no_error();
    }
