...
namespace components::vector::arrow {
    ...
    arrow_array_schema_wrapper_t::get_next_chunk(std::pmr::memory_resource* resource) {
        // Global new, like the std::make_shared it replaces: the wrapper outlives this call's
        // `resource` (vector buffers keep it alive past this scan step), so it must not bind to it.
