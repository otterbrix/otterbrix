...
TEST_CASE("components::vector::data_chunk_to_arrow") { {
    ...
    for (size_t i = 0; i < chunk_size; i++) {
        ...
        // array_string
        {
            ...
            arr.reserve(array_size);
            for (size_t j = 0; j < array_size; j++) {
                arr.emplace_back(&resource, std::string{"long_string_with_index_" + std::to_string(i * array_size + j)});
            }
        ...
        // list_string
        {
            ...
            list.reserve(list_length(i));
            for (size_t j = 0; j < list_length(i); j++) {
                list.emplace_back(&resource, std::string{"long_string_with_index_" + std::to_string(i * list_length(i) + j)});
            }
