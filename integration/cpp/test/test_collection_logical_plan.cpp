...
TEST_CASE("integration::cpp::test_collection::logical_plan") {
    ...
    INFO("insert") {
        ...
        {
            auto session = otterbrix::session_id_t();
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), ins, nullptr});
    ...
    INFO("find") {
        {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), agg, nullptr});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == kNumInserts);
        }
        {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), agg, params});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 10);
        }
        {
    ...
    INFO("group by boolean") {
        ...
        auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), aggregate, nullptr});
        REQUIRE(cur->is_success());
        REQUIRE(cur->size() == 2);
    ...
    INFO("delete") {
        ...
        }
        {
            ...
            auto params = logical_plan::make_parameter_node(dispatcher->resource());
            params->add_parameter(id_par{1}, types::logical_value_t(dispatcher->resource(), 90));
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), del, params});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 10);
        }
        {
    ...
    INFO("delete using") {
        ...
        {
            auto session = otterbrix::session_id_t();
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), del, nullptr});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 90);
        }
    }
    ...
    INFO("update from") {
        ...
        auto scan_agg = logical_plan::make_node_aggregate(dispatcher->resource(),
                                                          core::dbname_t{table_database_name},
                                                          core::relname_t{table_other_collection_name});
        auto scan_cur = dispatcher->execute_plan(scan_session, logical_plan::execution_plan_t{dispatcher->resource(), scan_agg, nullptr});
        ...
        vector::data_chunk_t data = std::move(scan_cur->chunk_data());
        {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), update, params});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 10);
        }
        {
    ...
    INFO("join with outside data") {
        ...
        }
        {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), ins_left, nullptr});
        }
        {
            auto session = otterbrix::session_id_t();
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), ins_right, nullptr});
        }
        INFO("right is raw data") {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), join, nullptr});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 26);
        ...
        INFO("both are raw data with complex join expr") {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), join, params});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 13);
        ...
        INFO("join raw data with aggregate") {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), aggregate, params});
            REQUIRE(cur->is_success());
            REQUIRE(cur->size() == 13);
        ...
        INFO("just raw data") {
            ...
            auto cur = dispatcher->execute_plan(session, logical_plan::execution_plan_t{dispatcher->resource(), node, nullptr});
            REQUIRE(cur->is_success());
