...
// services/collection/executor.cpp already calls check_collection_exists and answers either
// the IF NOT EXISTS no-op or core::error_code_t::table_already_exists. What was missing is
// its input: check_collection_exists is a pure read of the plan's resolved catalog entries
// (services/dispatcher/validate_logical_plan.cpp), which come from register_plan_targets
// walking the tree via target_names_of and skipping any node whose relname is empty
// (enrich_logical_plan.cpp, `if (relname.empty()) continue`). The create_collection_t arm of
// target_names_of returned an EMPTY relname, so CREATE TABLE registered its namespace and
// nothing else; the existence check always answered "does not exist" and the duplicate was
// written.
//
// Not a cosmetic wrong verdict: pg_class has no unique index on (relname, relnamespace) --
// system_table_schemas.cpp declares the columns not-null, not unique -- so the second CREATE
// appends a second row for the same name under a new oid with new storage, and
// operator_resolve_table binds the name to whichever row its scan reaches first.
//
// `inline_constraint_form_is_refused_too` below localises the defect: an inline constraint
// hangs a create_constraint_t child off the create node, and THAT node's target_names_of does
// name the table -- so the identical statement was refused or accepted depending on whether a
// column happened to carry a PRIMARY KEY.
