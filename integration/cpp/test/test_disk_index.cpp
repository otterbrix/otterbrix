...
TEST_CASE("integration::cpp::test_disk_index::sql_after_restart") {
    ...
    INFO("phase 1: create and fill via SQL") {
...

        {
            ...
            query << "INSERT INTO TestDatabase.TestCollection (count, _id, count_str, count_double, count_bool, "
                     "count_array) VALUES ";
            for (int num = 1; num <= kDocuments; ++num) {
                query << "(" << num << ", '" << gen_id(num, dispatcher->resource()) << "', '" << num << "', "
                      << (num + 0.1) << ", " << ((num % 2 != 0) ? "true" : "false") << ", ARRAY[0, 0, 0, 0, 0]"
                      << ")" << (num == kDocuments ? ";" : ", ");
            }
