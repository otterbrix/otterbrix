...
// Invariant under test: a secondary index contains exactly the non-NULL keys of the live rows. A
// NULL key must never enter an index. Before the fix, three distinct failure modes existed,
// selected by where the NULL sat and the column's type:
...
//               the caller is told it succeeded — every row after the NULL is missing.
//   * INVENT  — a leading NULL latches stored_type_ = NA, the cast becomes a no-op, and the
//               b-tree fills with mixed NA/typed keys; operator< returns false for NA regardless
//               of the right operand, so NA is "equivalent" to everything — not a strict weak
//               ordering, and lookups return rows that do not match.
//   * CRASH   — on a TEXT column, comparing a typed value against an NA dereferences a null string.
// Every query below runs against an indexed and an unindexed twin with identical data and checks
// the absolute count: some of these bugs make both tables wrong, so cross-checking alone would
// pass on two identically wrong answers. Disk and WAL are ON deliberately — the CREATE INDEX
// backfill branch is skipped with no disk address, which would make half this matrix vacuous.
...
namespace {
    ...
    // Assumes the standard fixture shape: x=5 / x=NULL / x=0.
    template<typename Dispatcher>
    void battery(Dispatcher* d, const std::string& idx_table, const std::string& plain_table) {
        ...
        both(d, idx_table, plain_table, "x < 1", 1);
...
TEST_CASE("integration::cpp::idx_null::backfill_null_middle") {
    ...
    REQUIRE(run(d, "CREATE INDEX ib2 ON ix.b2i (x);").ok);
...
TEST_CASE("integration::cpp::idx_null::backfill_separate_inserts_null_middle") {
    ...
    REQUIRE(run(d, "CREATE INDEX ib1 ON ix.b1i (x);").ok);
...
TEST_CASE("integration::cpp::idx_null::delete_null_and_value_rows") {
    ...
    for (auto* t : {"ix.e3i", "ix.e3p"}) {
        ...
        REQUIRE(run(d, std::string("INSERT INTO ") + t + " (id,x) VALUES (2,NULL);").ok);
        REQUIRE(run(d, std::string("INSERT INTO ") + t + " (id,x) VALUES (3,0);").ok);
        REQUIRE(run(d, std::string("DELETE FROM ") + t + " WHERE id = 2;").ok);
    }
    ...
    for (auto* t : {"ix.e3i", "ix.e3p"}) {
        REQUIRE(run(d, std::string("DELETE FROM ") + t + " WHERE id = 3;").ok);
    }
    both(d, "ix.e3i", "ix.e3p", "x = 0", 0);
...
TEST_CASE("integration::cpp::idx_null::null_constant_predicates") {
    // Regression guard: a NULL constant makes every comparison UNKNOWN, so all of these return 0
    // rows with or without an index — catches the zonemap or per-row compare ever treating a NULL
    // constant as an ordinary value.
