...
static const std::string udf_name = "concat";
...
std::unique_ptr<aggregate_function> make_concat_func() {
    ...
    auto fn = std::make_unique<aggregate_function>(udf_name, arity::unary(), doc, 1);
...
TEST_CASE("integration::cpp::test_udfs::aggregate") {
    ...
    INFO("unregister udf") {
        {
            auto session = otterbrix::session_id_t();
            bool result = dispatcher->unregister_udf(session, udf_name);
            REQUIRE(result);
        }
        // Trying to create same function will result in error
        {
