...
namespace otterbrix {
    ...
    bool python_import_cache_item_t::load_succeeded() const { return load_succeeded_; }

    inline bool python_import_cache_item_t::is_loaded() const { return object.ptr() != nullptr; }
    ...
    void python_import_cache_item_t::load_module(python_import_cache_t& cache) {
        try {
            ...
            if (is_required()) {
                throw std::runtime_error("Required module " + name +
                                         " failed to import, due to the following Python exception:\n" + e.what());
            }
            object = nullptr;
            return;
        }
