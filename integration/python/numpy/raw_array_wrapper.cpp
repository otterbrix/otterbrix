...
// (Previously this was two separate switches: GetNumpyTypeWidth and otter_brix_to_numpy_dtype.)
// Returns false for unsupported types; the human-readable error is composed by the caller that
// owns a memory_resource (otter_brix_to_numpy_dtype / initialize).
bool get_numpy_type_info(const complex_logical_type &type, numpy_type_info &out) {
	switch (type.type()) {
...
raw_array_wrapper_t::raw_array_wrapper_t(const complex_logical_type &type) : data(nullptr), type(type), count(0) {
	...
	type_width = get_numpy_type_info(type, info) ? info.width : 0;
}

core::result_wrapper_t<std::string> raw_array_wrapper_t::otter_brix_to_numpy_dtype(std::pmr::memory_resource *resource, const complex_logical_type &type) {
	numpy_type_info info{};
	if (!get_numpy_type_info(type, info)) {
...
core::error_t raw_array_wrapper_t::initialize(std::pmr::memory_resource *resource, idx_t capacity) {
	auto dtype = otter_brix_to_numpy_dtype(resource, type);
	if (dtype.has_error()) {
		return dtype.error();
	}
...
void raw_array_wrapper_t::resize(idx_t new_capacity) {
