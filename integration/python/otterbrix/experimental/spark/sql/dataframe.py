...
import otterbrix
from otterbrix import ColumnExpression, Expression #,StarExpression
...
from .type_utils import otterbrix_to_spark_schema
...
class DataFrame:
    ...
    def withColumnRenamed(self, columnName: str, newName: str) -> "DataFrame":
        ...
        for x in self.relation.columns:
            col = ColumnExpression(x, SparkContext._active_spark_context)
        ...
        if columnName in self.relation:
            ...
            for x in self.relation.columns:
                if x.casefold() == columnName.casefold():
                    ...
                    cols.append(ColumnExpression(x, SparkContext._active_spark_context))
        else:
    ...
    def select(self, *cols) -> "DataFrame":
        ...
        if isinstance(cols, list):
            projections = [
                x.expr if isinstance(x, Column) else ColumnExpression(x, SparkContext._active_spark_context) for x in cols
            ]
        else:
            projections = [
    ...
    def columns(self) -> List[str]:
        ...
        ['age', 'name']
        """
        return [f for f in self.relation.columns]
    ...
    def toDF(self, *cols) -> "DataFrame":
        ...
        existing_columns = [ColumnExpression(x, SparkContext._active_spark_context) for x in existing_columns]
        projections = [
