...
namespace otterbrix {
    static void initialize_read_only_properties(py::class_<py_relation_t> &m) {
            m.def_property_readonly("columns", &py_relation_t::columns,
                               "Return a list containing the names of the columns of the relation.")
            .def_property_readonly("types", &py_relation_t::column_types,
                               "Return a list containing the types of the columns of the relation.")
            .def_property_readonly("dtypes", &py_relation_t::column_types,
                               "Return a list containing the types of the columns of the relation.");
    }
    ...
    static void initialize_consumers(py::class_<py_relation_t> &m) {
        m.def("fetchone", &py_relation_t::fetch_one, "Execute and fetch a single row as a tuple")
            .def("fetchmany", &py_relation_t::fetch_many, "Execute and fetch the next set of rows as a list of tuples", py::arg("size") = 1)
    ...
    void py_relation_t::initialize(py::handle& m) {
        ...
        relation_module.def("project", &py_relation_t::project, 
                "Project the relation object by the projection in project_expr");
        relation_module.def("select", &py_relation_t::project, 
                "Project the relation object by the projection in project_expr");
        relation_module.def("filter", &py_relation_t::filter, "Filter the relation object by the filter in filter_expr",
                py::arg("filter_expr"));

        relation_module
            .def("group", &py_relation_t::group, "Group fields with aggregation expressions")
            ...
            .def("cross", &py_relation_t::cross, "Create cross/cartesian product of two relational objects", py::arg("other_rel"));
