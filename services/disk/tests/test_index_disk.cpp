...
TEST_CASE("services::disk::index_disk::int32") {
    ...
    for (int i = 1; i <= 100; ++i) {
        index.insert(logical_value_t(int64_t(i)), static_cast<size_t>(i));
    }

    REQUIRE(index.find(logical_value_t(1l)).size() == 1);
    REQUIRE(index.find(logical_value_t(1l)).front() == 1);
    REQUIRE(index.find(logical_value_t(10l)).size() == 1);
    REQUIRE(index.find(logical_value_t(10l)).front() == 10);
    REQUIRE(index.find(logical_value_t(100l)).size() == 1);
    REQUIRE(index.find(logical_value_t(100l)).front() == 100);
...
TEST_CASE("services::disk::index_disk::uint32") {
    ...
    for (int i = 1; i <= 100; ++i) {
        index.insert(logical_value_t(uint64_t(i)), static_cast<size_t>(i));
    }

    REQUIRE(index.find(logical_value_t(1ul)).size() == 1);
    REQUIRE(index.find(logical_value_t(1ul)).front() == 1);
    REQUIRE(index.find(logical_value_t(10ul)).size() == 1);
    REQUIRE(index.find(logical_value_t(10ul)).front() == 10);
    REQUIRE(index.find(logical_value_t(100ul)).size() == 1);
    REQUIRE(index.find(logical_value_t(100ul)).front() == 100);
...
TEST_CASE("services::disk::index_disk::double") {
    ...
    for (int i = 1; i <= 100; ++i) {
        index.insert(logical_value_t(double(i)), static_cast<size_t>(i));
    }

    REQUIRE(index.find(logical_value_t(1.)).size() == 1);
    REQUIRE(index.find(logical_value_t(1.)).front() == 1);
    REQUIRE(index.find(logical_value_t(10.)).size() == 1);
    REQUIRE(index.find(logical_value_t(10.)).front() == 10);
    REQUIRE(index.find(logical_value_t(100.)).size() == 1);
    REQUIRE(index.find(logical_value_t(100.)).front() == 100);
...
TEST_CASE("services::disk::index_disk::multi_values::int32") {
    ...
    for (int i = 1; i <= 100; ++i) {
        for (int j = 0; j < 10; ++j) {
            index.insert(logical_value_t(int64_t(i)), static_cast<size_t>(1000 * j + i));
        }
    }
    ...
    for (int i = 2; i <= 100; i += 2) {
        for (int j = 5; j < 10; ++j) {
            index.remove(logical_value_t(int64_t(i)), static_cast<size_t>(1000 * j + i));
        }
    }
