...
// NOTE: the helper API carries no restrict/cascade distinction — despite its name, this case
// only verifies that a committed drop removes the namespace rows.
...
TEST_CASE("services::disk::pg_depend::test_no_cycle_linear_chain") {
    ...
    constexpr components::catalog::oid_t OID_A = 100;
    constexpr components::catalog::oid_t OID_B = 200;
    constexpr components::catalog::oid_t OID_C = 300;
...
// Regression: child_name.schema/database mix-up in fk_validate_parent_delete.
