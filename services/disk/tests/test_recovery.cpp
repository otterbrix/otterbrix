        ...
        for (const auto& chunk : batches) {
            ...
            for (std::uint64_t i = 0; i < chunk.size(); ++i) {
                ...
                const auto attname = chunk.value(2, i).is_null()
                                         ? std::string{}
                                         : std::string(chunk.get_value_not_null<std::string_view>(2, i));
                const auto atttypid =
                    chunk.value(3, i).is_null()
                        ? components::catalog::INVALID_OID
                        : static_cast<components::catalog::oid_t>(chunk.get_value_not_null<std::uint32_t>(3, i));
                const auto refcount = chunk.get_value_not_null<std::int64_t>(6, i);
                REQUIRE(refcount == 1);
                if (attname == "a") {
