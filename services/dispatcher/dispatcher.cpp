...
namespace services::dispatcher {
    ...
    manager_dispatcher_t::execute_plan(components::session::session_id_t session,
        ...
        switch (logic_plan->type()) {
            ...
            case node_type::create_constraint_t: {
                ...
                // The real work is dispatched via execute_ddl_inline → execute_ddl_plan.
        ...
        if (result->is_success()) {
            ...
            const bool is_ddl = t == node_type::create_database_t || t == node_type::drop_database_t ||
                                 ...
                                 t == node_type::alter_table_t || t == node_type::create_constraint_t;
            if (is_ddl) {
                trace(log_, "manager_dispatcher_t::execute_plan: DDL {} via inline pipeline",
                      to_string(t));
                auto ddl_cursor = co_await execute_ddl_inline(session, logic_plan, txn_data, view);
    ...
    manager_dispatcher_t::commit_transaction(components::session::session_id_t session) {
        ...
        // Mirrors the auto-commit path in execute_plan_impl (execute_ddl_inline).
    ...
    manager_dispatcher_t::execute_ddl_inline(components::session::session_id_t session,
        ...
        switch (logical_plan->type()) {
            ...
            case node_type::create_collection_t: {
                ...
                auto rns = co_await std::move(rnf);
                if (rns.found) {
                    // ddl_create_table must persist the *resolved* column types so
                    // pg_attribute.atttypspec round-trips. Use the original definitions
                    // when no columns (computed table case), otherwise use the same
                    // resolved set storage uses above.
                    std::vector<components::table::column_definition_t> ddl_cols =
                        cc->column_definitions().empty()
                            ? cc->column_definitions()
                            : cc->column_definitions();
                    if (!ddl_cols.empty()) {
                        // Resolve UDT references in ddl_cols (mirrors storage path above).
            ...
            case node_type::drop_collection_t: {
                ...
                if (rns.found) {
                    ...
                    if (rt.found) {
                        ...
                        if (drop_r.failed())
                            co_return make_ddl_error_cursor(resource(), drop_r);
                    }
                }
                break;
            }
            ...
            case node_type::create_constraint_t: {
                ...
                auto [_rn2, rnf2] = actor_zeta::send(disk_address_,
                                                      &disk::manager_disk_t::resolve_namespace,
                                                      ddl_ctx,
                                                      logical_plan->database_name(),
                                                      std::uint64_t{0});
                ...
                if (rns2.found) {
                    auto [_rt2, rtf2] = actor_zeta::send(disk_address_,
                                                          &disk::manager_disk_t::resolve_table,
                                                          ddl_ctx,
                                                          rns2.oid,
                                                          logical_plan->collection_name(),
        ...
        // commit_txn → WAL: persists the txn marker with FULL fsync mode so DDL records
        // are durable before this query returns.
        if (wal_address_ != actor_zeta::address_t::empty_address()) {
        ...
        if (wal_address_ != actor_zeta::address_t::empty_address() &&
            disk_address_ != actor_zeta::address_t::empty_address()) {
            auto [_ac, acf] = actor_zeta::send(wal_address_,
            ...
            if (ac_wal_id > wal::id_t{0}) {
                ...
                if (safe_trunc > wal::id_t{0}) {
                    auto [_tr, trf] = actor_zeta::send(wal_address_,
                                                        &wal::manager_wal_replicate_t::truncate_before,
                                                        session, safe_trunc);
        ...
        // Flip MVCC state on storage rows just appended under txn_data.transaction_id.
        // Order: WAL commit_txn (durability) → txn_manager.commit (returns commit_id) →
        // disk.commit_pg_catalog_appends (flips insert_id from txn_id to commit_id so
        // scan_committed visibility is consistent). No-op when txn_data.transaction_id == 0.
        if (txn_data.transaction_id != 0) {
            uint64_t commit_id = txn_manager_.commit(session);
            if (disk_address_ != actor_zeta::address_t::empty_address() && commit_id > 0) {
                components::execution_context_t cpa_ctx{session, txn_data, {}};
                auto [_cpa, cpaf] = actor_zeta::send(disk_address_,
                                                      &disk::manager_disk_t::commit_pg_catalog_appends,
                                                      cpa_ctx, commit_id);
